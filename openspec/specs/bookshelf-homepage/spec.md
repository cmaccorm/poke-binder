...
- **AND** the binders wrap to new shelf rows depending on available screen width
...
- **AND** the bottom of each binder card visually rests on or slightly overlaps the shelf plank
...
- **AND** empty space on a shelf is still rendered as a continuous black/charcoal wooden plank